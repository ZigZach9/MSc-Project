...
def addMHRecord(aspect):
    ...
    score = 0
    # Get values from webpage. If self-esteem has 10 items, whereas rest have 8.
    if aspect == "self_esteem":
        for i in range(1,11):
            # Check to see if what user entered can be converted to int. If not it means user has not given an answer.
            try:
                itemScore = int(request.form.get(str(i)))
            except:
                return 1
        # Calculate score
            score += itemScore
...
def depression():
    if request.method == "POST":
        # If a 1 was returned, it means that not all questions were answered by the user
